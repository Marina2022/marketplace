...
const RegionItem = ({region, currentRegion, defineRegion}) => {

  const isActive = region?.regionId === currentRegion?.regionId;

  return (
    <li onClick={() => defineRegion(region)} key={region.regionId}
        className={`${s.regionItem} ${isActive ? s.regionItemActive : ''} `}>
      {
        region.name
      }
      ...
      {
        isActive && (
          <svg className={s.activeIndicator} width="15" height="15" viewBox="0 0 15 15" fill="none"
