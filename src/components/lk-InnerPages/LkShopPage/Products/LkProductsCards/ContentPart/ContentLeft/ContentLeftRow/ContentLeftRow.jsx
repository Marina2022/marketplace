...
import LinkedProducts
  from "@/components/lk-InnerPages/LkShopPage/Products/LkProductsCards/ContentPart/ContentLeft/ContentLeftRow/LinkedProducts/LinkedProducts.jsx";

const ContentLeftRow = ({product, checkedProducts, setCheckedProducts, collapsedProducts, setCollapsedProducts}) => {
  ...
  const handleCheck = ()=>{
    ...
    let checkedProductsToSet
    if (tempCheckedProducts.includes(product.productVariantId)){
      checkedProductsToSet = tempCheckedProducts.filter(item=>item!==product.productVariantId)
      setCheckedProducts(checkedProductsToSet)
    } else {
  ...
  const handleCollapse = ()=>{
    ...
    let collapsedProductsToSet
    if (tempCollapsedProducts.includes(product.productVariantId)){
      collapsedProductsToSet = tempCollapsedProducts.filter(item=>item!==product.productVariantId)
      setCollapsedProducts(collapsedProductsToSet)
    } else {
    ...
    <div className={s.leftRowWrapper}>
      <div className={s.leftRow}>
        <div className={s.photoAndCheck} onClick={handleCheck}>
          {
            checked ? (
                <svg className={s.checkedSvg} width="16" height="17" viewBox="0 0 16 17" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <div className={s.rightPartOfLeftColumn}>
...

          {
            hasLinked && <div className={s.unitedBlock} onClick={handleCollapse} > 
              ...
              <svg className={collapsed ? s.collapseIconReversed : s.collapseIcon} width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path
      ...
      {
        hasLinked && !collapsed && <LinkedProducts linkedProducts={product.linkedProducts}/>
      }
    </div>
