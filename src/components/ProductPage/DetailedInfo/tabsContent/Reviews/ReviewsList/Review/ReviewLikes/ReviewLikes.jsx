...
import {getLikes, getReviews, updateLikes} from "@/store/reviewsSlice.js";
...
const ReviewLikes = ({ review}) => {
  ...
  const myReview = reviews.find(item=>review.reviewId === item.reviewId)
  const likes = myReview.likes
  const dislikes = myReview.dislikes
  ...
  // этот объект в экшн будем посылать при нажатии на кнопку лайк/дизлайк
  let newLikesObj = {
    reviewLikes: [],
    reviewDislikes: [],
    questionLikes: [],
...
// объект будет отправлен на api opinion
  ...
  useEffect(() => {
    ...
    if (likesObj) {
      newLikesObj = {...likesObj}

      if (newLikesObj.reviewLikes.includes(review.reviewId)) {
        setUserLikesIt(true)
      } else {
        setUserLikesIt(false)
      }

      if (newLikesObj.reviewDislikes.includes(review.reviewId)) {
        setUserDislikesIt(true)
      } else {
        setUserDislikesIt(false)
      }

      // если в LS или на сервере нет лайков/дизлайков по этому отзыву, likesObj - будет null
    } else {
      setUserLikesIt(false)
      setUserDislikesIt(false)
    }
  ...
  const handleLikeClick = async () => {
    ...
    dispatch(updateLikes({newLikesObj, opinionApiObj}))    
  }
    ...
    <div className={s.wrapper}>
      ...
      <div className={s.likesGroup}>
        <button onClick={handleDislikeClick} className={s.likeBtn}>
          <svg className={userDislikesIt ? s.likeActive : s.like} width="24" height="24" viewBox="0 0 24 24" fill="3E5067"
               xmlns="http://www.w3.org/2000/svg">
            <path
        ...
        <span className={s.text}> {dislikes !== 0 ? '- ' +  dislikes : '' +  dislikes} </span>
      </div>
    </div>
  );
};
