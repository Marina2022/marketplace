...
import {getCurrentChatRequest, setCurrentChatRequestInfo} from "@/store/chatSlice.js";
  ...
  useEffect(() => {
    ...
    const getRequestDetails = async () => {
      try {
        ...
        const mediaFileId = data.previewMediaFileId
        let imgUrl = noPhotoImg

        // если есть картинка — пробуем получить url
        if (mediaFileId) {
          const now = Date.now()
          ...
          if (cached && new Date(cached.expiresAt).getTime() > now) {
            ...
            try {
              ...
              const url = pictures?.items?.[currentRequestId]?.url
