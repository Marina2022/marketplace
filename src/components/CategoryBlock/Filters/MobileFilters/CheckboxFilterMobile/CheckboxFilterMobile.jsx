import s from './CheckboxFilterMobile.module.scss';
import {useState} from "react";
import ShowMoreModal from "@/components/CategoryBlock/Filters/MobileFilters/ShowMoreModal/ShowMoreModal.jsx";
import CheckboxFilterMobileItem
  from "@/components/CategoryBlock/Filters/MobileFilters/CheckboxFilterMobile/CheckboxFilterMobileItem/CheckboxFilterMobileItem.jsx";
  ...
  return (
      <li className={s.filterItem}>
        ...
        <ul className={s.filtersList}>
          {
            valuesToShow.map((value, i) => <CheckboxFilterMobileItem
                key={i}
                valueObject={value}
            ...
            <>

              {
                  activeValues.length > 0 && <ul className={s.filtersListFull}>
                    ...
                    {
                      activeValues.map((value, i) => <CheckboxFilterMobileItem
                          key={i}
                          valueObject={value}
                          currentFilters={currentFilters}
                          setCurrentFilters={setCurrentFilters}
