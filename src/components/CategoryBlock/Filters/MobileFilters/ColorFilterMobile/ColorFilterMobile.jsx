...
import ColorFilterMobileItemFull
  from "@/components/CategoryBlock/Filters/MobileFilters/ColorFilterMobile/ColorFilterMobileItemFull/ColorFilterMobileItemFull.jsx";
import ColorFilterMobileItem
...
const ColorFilterMobile = ({filter, currentFilters, setCurrentFilters}) => {
  ...
  const valuesToShow = filterSettingsSorted.slice(0, 10)
  ...
  return (
      <li className={s.filterItem}>
        <div className={s.topWrapper}>
          ...
          {filterSettingsSorted.length > 10 && (
        ...
        <ul className={s.filtersList}>
          {
            valuesToShow.map((value, i) => <ColorFilterMobileItem
                key={i}
                valueObject={value}
            ...
            <>

              {
                  activeValues.length > 0 && <ul className={s.filtersListFull}>
                    <button className={s.deleteBtn} onClick={onDeleteAllClick}>
                    ...
                    {
                      activeValues.map((value, i) => <ColorFilterMobileItem
                          key={i}
                          valueObject={value}
                          currentFilters={currentFilters}
                          setCurrentFilters={setCurrentFilters}
              ...
              <ul>
                {
                  filterSettingsSorted.map((value, i) => <ColorFilterMobileItemFull
